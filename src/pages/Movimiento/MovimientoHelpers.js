...
export const getBancoNombre = (m) =>
  m?.baN_Nombre ?? m?.bAN_Nombre ?? m?.ban_nombre ?? "";

export const getSimboloMoneda = (m) =>
  m?.tmO_Simbolo ?? m?.tMO_Simbolo ?? m?.tmo_simbolo ?? "Q";

export const getMonedaDescripcion = (m) =>
  m?.tmO_Descripcion ?? m?.tMO_Descripcion ?? m?.tmo_descripcion ?? "";
