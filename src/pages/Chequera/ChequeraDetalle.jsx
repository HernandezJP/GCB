...
import { ArrowLeft, BookOpen, FileText, AlertCircle, Loader, CheckCircle } from 'lucide-react';
...
const estadoCheqPill = (e) => {
    const m = { Emitido:'chq-pill-blue', Cancelado:'chq-pill-gray', Depositado:'chq-pill-green', Activo:'chq-pill-green' };
...
const ChequeraDetalle = ({ chequera, onBack }) => {
    ...
    const desde    = getQDesde(chequera);
    const hasta    = getQHasta(chequera);
    const usados   = getQUsados(chequera);
    ...
    useEffect(() => {
        ...
        getChequesPorCuenta(cuentaId)
            .then(data => {
                // Filtrar solo los de esta chequera
                const qId = getQId(chequera);
                const filtrados = (data ?? []).filter(c =>
                    g(c,'cHQ_Chequera','chQ_Chequera','CHQ_Chequera') === qId
                );
            ...
            <div className="detalle-card">
                <div className="detalle-header">
                    ...
                    <div>
                        ...
                        <p className="detalle-subtitle">
                            {banco}
                            {numCuenta && (
                                <> · <code style={{ fontFamily:'monospace', fontWeight:700, color:'#0284c7' }}>{numCuenta}</code></>
                            )}
                        </p>
                    </div>
                ...
                <div style={{ padding:'16px 24px', borderBottom:'1px solid #f1f5f9' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:8 }}>
                        ...
                        <span style={{ fontWeight:600 }}>{usados} usados de {total} ({pct}%)</span>
                    </div>
                ...
                <div className="detalle-stats">
                    {[
                        { label:'Rango',         val:<code className="chq-num">{String(desde).padStart(5,'0')} — {String(hasta).padStart(5,'0')}</code> },
                        { label:'Total cheques', val: total },
                        { label:'Usados',        val: usados },
                        { label:'Disponibles',   val: <span style={{ fontWeight:700, color:disp===0?'#b91c1c':disp<5?'#92400e':'#15803d' }}>{disp}</span> },
                ...
                {disp === 0 && (
                    <div style={{ margin:'0 20px 16px', background:'#fffbeb', border:'1px solid #fde68a', borderRadius:8, padding:'10px 14px', fontSize:12, color:'#92400e', display:'flex', alignItems:'center', gap:8 }}>
                        <AlertCircle size={14}/> Esta chequera está agotada. Registra una nueva para seguir emitiendo cheques.
                    </div>
                )}

                {/* Tab cheques de esta chequera */}
                <div style={{ borderTop:'1px solid #e2e8f0' }}>
                    ...
                    {loading
                        ? <div className="chq-loading"><Loader size={18} color="#0284c7"/><span>Cargando cheques...</span></div>
                        : cheques.length === 0
                            ? <div className="chq-empty"><FileText size={28} color="#e2e8f0"/><span>No hay cheques emitidos con esta chequera.</span></div>
                            : (
                                <div style={{ overflowX:'auto' }}>
                                    <table className="chq-table">
                                        ...
                                        <tbody>
                                            {cheques.map((c,i) => {
                                                const est    = g(c,'estadoCheque','EstadoCheque','ESC_Descripcion') ?? '';
                                                const monto  = Math.abs(g(c,'mOV_Monto','moV_Monto','MOV_Monto') ?? 0);
                                                ...
                                                return (
                                                    <tr key={i}>
                                                        <td><code className="chq-num">{numCh}</code></td>
                                                        ...
                                                        <td><span className={chq-pill ${estadoCheqPill(est)}}>{est||'—'}</span></td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
